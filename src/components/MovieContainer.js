...
export default async function MovieContainer({
  ...
  const data = endpoint ? await fetcher(endpoint, queryParams) : null;

  return (
    <div className="p-5">
      {isVertical && (
        <h2 className="text-4xl font-bold mb-5">
          Results for {typeName ? typeName : title?.replace("-", /\s+/g)}
        </h2>
      )}
      <div className="relative flex items-center justify-between pb-5 border-b border-b-gray-600 mb-5">
        ...
        {!title?.includes("type") && (
          <Link
            href={{ pathname: `/viewmore/${title?.replace(/\s+/g, "-")}` }}
      ...
      {isVertical ? (
        <div className="flex flex-col max-h-full items-center lg:items-start justify-between gap-10 ">
          {data?.results?.map((movie) => {
            return (
              <div className="flex flex-col lg:flex-row items-center gap-5 w-11/12 2xl:w-7/12" key={movie?.id}>
